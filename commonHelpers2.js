import{g as G,s as u,a as M,b as Q,d as K,e as V}from"./assets/get-api-50b9f757.js";import{a as U,P as W,l as Y}from"./assets/vendor-8c378b12.js";class X{constructor(){this.URL="https://food-boutique.b.goit.study/api",this.searchQuery="",this.category="",this.currentPage=1,this.perPage=90}paramsFromApi(e){return Object.entries(e).map(([o,s])=>`${encodeURIComponent(o)}=${encodeURIComponent(s)}`).join("&")}getFoodList(){const e={keyword:this.searchQuery,category:this.category,page:this.currentPage,limit:this.perPage},o=this.paramsFromApi(e);return U.get(`${this.URL}/products?${o}`).then(s=>s.data)}}class Z{updateLsWithList(e,o){localStorage.getItem("options")||this.defaultApiOptions(),localStorage.setItem("products",JSON.stringify(e.results)),localStorage.setItem("options",JSON.stringify(o))}defaultApiOptions(){const e={keyword:null,category:null,page:1,limit:6};localStorage.setItem("options",JSON.stringify(e))}}function B(t){const{_id:e,name:o,img:s,category:c,size:n,price:a,popularity:r}=t;return`
      ...
      `}const E=document.querySelector(".list-prod"),tt=document.querySelector(".filter-form"),f=document.querySelector(".filter-select"),p=new X,$=new Z;G().then(t=>{t.forEach(o=>{const s=document.createElement("option");s.value=o,s.textContent=o,f.appendChild(s)});const e=et();f.appendChild(e)});function et(){const t=document.createElement("option");return t.textContent="Show All",t.value="show-all",t}tt.addEventListener("submit",function(t){t.preventDefault();const e=t.target.elements.searchQuery.value;console.log("Search Value:",e);try{let o=JSON.parse(localStorage.getItem("options"))||{};o.keyword=e,p.searchQuery=e,console.log("foodApi.searchQuery:",p.searchQuery),p.getFoodList().then(function(s){$.updateLsWithList(s,o);const c=JSON.parse(localStorage.getItem("products")),n=c.length;console.log(n),D(c)}).catch(function(s){console.error("Error fetching food list:",s.message)})}catch(o){console.error("Error:",o.message)}});f.addEventListener("change",function(){const t=f.value;try{let e=JSON.parse(localStorage.getItem("options"))||{};t==="show-all"?(localStorage.removeItem("products"),$.defaultApiOptions(),document.getElementById("search").value="",categoriesFilter()):(e.category=t,p.category=t,p.getFoodList().then(function(o){$.updateLsWithList(o,e);const s=JSON.parse(localStorage.getItem("products")),c=s.length;D(s)}).catch(function(o){console.error("Error fetching list:",o.message)}))}catch(e){console.error("Error:",e.message)}});function D(t){let e=1;const o=1,s=90,c=JSON.parse(localStorage.getItem("products"));E.innerHTML="";let n,a;isNaN((o-1)*s)?n=0:n=(o-1)*s,a=+e*+s,console.log(n),console.log(a);try{if(c&&t>=1){let r=(t-1)*8;const i=c.slice(n+r,a+r);for(let l=0;l<i.length;l+=1){const b=B(i[l]);E.insertAdjacentHTML("beforeend",b)}}else if(c&&c.length>0){const r=c.slice(n,a);for(let i=0;i<r.length;i+=1){const l=B(r[i]);E.insertAdjacentHTML("beforeend",l)}}}catch(r){console.log(r)}}const y=document.getElementById("modalProduct"),ot=document.getElementById("closeModalProductBtn"),st=document.getElementById("addToCartBtn"),ct=document.getElementById("modalProductImage"),nt=document.getElementById("modalProductName"),at=document.getElementById("modalProductCategory"),rt=document.getElementById("modalProductSize"),it=document.getElementById("modalProductPopularity"),lt=document.getElementById("modalProductDescription"),dt=document.getElementById("modalProductPrice");function I(t){if(!t||!t.img){console.error("Product data is missing or incomplete.");return}ct.src=t.img,nt.textContent=t.name,at.innerHTML=`Category: <span id="priceText">$ ${t.category}</span>`,document.getElementById("priceText").style.color="black",rt.innerHTML=`Size: <span id="priceTexte">$ ${t.size}</span>`,document.getElementById("priceTexte").style.color="black",it.innerHTML=`Popularity: <span id="priceTex">$ ${t.popularity}</span>`,document.getElementById("priceTex").style.color="black",lt.textContent=`${t.desc}`,dt.textContent=`$ ${t.price}`,y.style.display="block",window.addEventListener("click",k)}function g(){document.body.style.overflow="",y.style.display="none",window.removeEventListener("click",k)}ot.addEventListener("click",g);window.addEventListener("click",k);document.addEventListener("keydown",function(t){t.key==="Escape"&&g()});function k(t){t.target===y&&g()}window.addEventListener("click",function(t){t.target===y&&g()});st.addEventListener("click",function(){g()});const O=document.querySelector(".list-prod"),C="added-item",F="firstGet";function P(t,e){localStorage.setItem(t,JSON.stringify(e))}function J(t){try{const e=localStorage.getItem(t);return e?JSON.parse(e):[]}catch(e){console.log(e)}}function ut(t,e){t.innerHTML=e}async function T(t){const e=ft(t);ut(O,e)}const v=J(F);console.log(v);const j=mt(v);console.log(j);const w=j.includes(v._id);console.log(w);O.addEventListener("click",pt);O.addEventListener("click",gt);function pt(t){const e=t.target.closest(".buy-btn");if(!e)return;const o=t.target.closest(".prod-item");console.log(o);const s=e.querySelectorAll("svg"),c=o.getAttribute("data-js-product-id");console.log(c);const n=J(F);if(console.log(n),n.find(a=>c===a._id)){const a=n.filter(r=>c!==r._id);P(C,a)}else{const a=products.find(r=>c===r._id);n.push(a),P(C,n)}s.forEach(a=>{a.classList.toggle("is-hidden")})}function gt(t){const e=t.target.closest(".prod-item"),o=t.target.closest(".buy-btn");if(!e||o)return;const s=e.getAttribute("data-js-product-id"),c=v.find(n=>s===n._id);I(c)}function mt(t=[]){return t.map(e=>e._id)}function ft(t){const e=w?"":"is-hidden",o=w?"is-hidden":"";return t.map(({_id:s,name:c,img:n,category:a,size:r,price:i,popularity:l})=>`
