import{g as p,S as F,b as q,m as N,s as g,d as X,e as Q,a as l,f as j,h as Z,i as tt,j as et}from"./assets/STORAGE-f69de671.js";import{P as ot,a as st,l as ct}from"./assets/vendor-8c378b12.js";document.getElementById("addToCartIcon");const nt=document.getElementById("removeFromIcon");nt.classList.add("visually-hidden");const $=document.getElementById("modalProduct"),at=document.getElementById("closeModalProductBtn"),E=document.getElementById("addToCartBtn"),L=document.getElementById("removeFrom"),rt=document.getElementById("modalProductImage"),dt=document.getElementById("modalProductName"),it=document.getElementById("modalProductCategory"),lt=document.getElementById("modalProductSize"),ut=document.getElementById("modalProductPopularity"),pt=document.getElementById("modalProductDescription"),mt=document.getElementById("modalProductPrice"),gt=document.querySelector(".modal-overlay");const V=document.getElementById("scrollToTopBtn");function M(t){if(!t||!t.img){console.error("Product data is missing or incomplete.");return}$.style.display="block",document.body.style.overflow="hidden",document.querySelector(".modal-overlay").style.display="flex",window.addEventListener("click",I),E.addEventListener("click",s),L.addEventListener("click",c);const e=p(F);let o=q(e,t);N(o,E,L);function s(){e.push(t),n(e)}function c(){e.splice(o,1),n(e)}function n(a){g(F,e),X(),a=q(e,t),N(a,E,L)}V.style.display="none";rt.src=t.img,dt.textContent=t.name,it.innerHTML=`Category: <span id="priceText"> ${t.category.replace(/_/g," ")}</span>`,document.getElementById("priceText").style.color="black",lt.innerHTML=`Size: <span id="priceTexte"> ${t.size}</span>`,document.getElementById("priceTexte").style.color="black",ut.innerHTML=`Popularity: <span id="priceTex"> ${t.popularity}</span>`,document.getElementById("priceTex").style.color="black",pt.textContent=`${t.desc}`,mt.textContent=`$ ${t.price}`,$.style.display="block",document.body.style.overflow="hidden",document.querySelector(".modal-overlay").style.display="block",window.addEventListener("click",I)}function C(){document.body.style.overflow="",$.style.display="none",document.querySelector(".modal-overlay").style.display="none",window.removeEventListener("click",I),isProductAdded=!1,V.style.display="block";}at.addEventListener("click",C);document.addEventListener("keydown",function(t){t.key==="Escape"&&C()});function I(t){t.target===gt&&C()}const T=document.querySelector(".list-prod"),D="added-item",G="firstGet";function k(t,e){localStorage.setItem(t,JSON.stringify(e))}function U(t){try{const e=localStorage.getItem(t);return e?JSON.parse(e):[]}catch(e){console.log(e)}}function ht(t,e){t.innerHTML=e}async function J(t){const e=bt(t);ht(T,e)}const v=U(G);console.log(v);const K=vt(v);console.log(K);const B=K.includes(v._id);console.log(B);T.addEventListener("click",ft);T.addEventListener("click",yt);function ft(t){const e=t.target.closest(".buy-btn");if(!e)return;const o=t.target.closest(".prod-item"),s=e.querySelectorAll("svg"),c=o.getAttribute("data-js-product-id");console.log(c);const n=U(G);if(console.log(n),n.find(a=>c===a._id)){const a=n.filter(r=>c!==r._id);k(D,a)}else{const a=products.find(r=>c===r._id);n.push(a),k(D,n)}s.forEach(a=>{a.classList.toggle("is-hidden")})}function yt(t){const e=t.target.closest(".prod-item"),o=t.target.closest(".buy-btn");if(!e||o)return;const s=e.getAttribute("data-js-product-id");v.find(c=>s===c._id),Q(s).then(c=>{M(c)}).catch(c=>{console.error(c)})}function vt(t=[]){return t.map(e=>e._id)}function bt(t){return t.map(({_id:e,name:o,img:s,category:c,size:n,price:a,popularity:r,is10PercentOff:d})=>{const u=B?"":"is-hidden",S=B?"is-hidden":"",Y=St(d);return`
